...
class TestRandomBoundingBox():
    ...
    def test_random_bounding_box_dtype_int(self,height, width):
        ...
        for _ in range(1000):
            ...
            assert all([isinstance(point, int) for point in [x1, x2, y1, y2]]), f"Dtypes are:{[type(point) for point in [x1, x2, y1, y2]]}"
...
class TestGetMask():
    ...
    @pytest.mark.parametrize("input_shape, expected_dtype",[
