...
class PixelDistanceLoss(tf.keras.losses.Loss):
    ...
    def __init__(
        self,
        globalBatchSize,
        calculationType="per_image",
        normalizeDepthChannel=False,
        MAE_MSE="mae",
            ...
            if self.normalizeDepthChannel:
                ...
                for i in range(img1.shape[-1]):
                    loss += (
                        ...
                        * (1.0 / self.globalBatchSize)
                    )

        elif self.calculationType == "per_image":
            loss = tf.reduce_mean(errorFunc(img1 - img2)) * (1.0 / self.globalBatchSize)
...
class StructuralSimilarityLoss(tf.keras.losses.Loss):
    ...
    def SSIM(self, tensor1, tensor2):
        ...
        return (1 - SSIM) * (1.0 / self.globalBatchSize)
